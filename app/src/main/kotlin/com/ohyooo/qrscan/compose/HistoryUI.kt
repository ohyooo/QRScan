...
import com.ohyooo.qrscan.ScanHistoryItem
...
fun HistoryUI(
    ...
    LazyColumn(
        ...
        items(history, key = { it.value }) { item ->
            Surface(
                modifier = Modifier
                    .fillMaxWidth()
                ...
                Column(modifier = Modifier.padding(18.dp)) {
                    ...
                    Row(
                        ...
                        if (item.canOpenLink) {
                            TextButton(onClick = { onOpenLink(item.value) }) {
...
private fun HistoryPreview() {
    QRScanTheme {
        HistoryUI(
            history = listOf(
                ScanHistoryItem("https://openai.com"),
                ScanHistoryItem("BEGIN:VCARD\nFN:Jane Doe\nTEL:+123456")
            ),
            onSelect = {},
            onOpenLink = {}
        )
